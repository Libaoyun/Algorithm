...
*/
         ...
         * 方法2：官方题解划线法：，使用二分查找，使用分割线划分两数组为左右两部分。
         * 两边元素分别为(m+n+1)/2个，这里加一是为了让左边数量>=右边元素数量
        ...
        // 如果i=0，那么j一定是一半但是交换后nums2长度大于一半;如果i就是nums1.length，那么j也不会是0因为第一个数组比第二个短
        ...
        while (left < right){
            ...
            // 而且这里二分查找本身就是为了寻找最合适的两分界线，再加上totalLeft = (m+n+1)/2;目标就是找到最大的符合条件的i,因此分界线只能有一条
